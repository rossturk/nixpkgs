...
from typing import Any, Dict, Iterator, List, Union, Optional, Callable, ContextManager, Tuple
import os
import tempfile
...
class Driver:
        ...
        with rootlog.nested("start all TPMs"):
            self.tpms = {
                tpm_data['machine_name']: Tpm(
                    tpm_data['swpath_binary_path'],
                    tpm_data['socket_path']
                )
    ...
    def test_symbols(self) -> Dict[str, Any]:
        ...
        if len(self.machines) == 1:
            ...
            (tpm_symbols["tpm_machine"],) = self.tpms
        vlan_symbols = {
